...
class TestRDFGeneration(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.t2wml_spec_path = './ground_truth/t2wml_spec_1.yaml'
        self.sparql_endpoint = 'https://dsbox02.isi.edu:8899/bigdata/namespace/wdq/sparql'
        self.results_path = './ground_truth/results_1.ttl'

    def test_rdf_generation(self, sheet_name: str = None):
        results = open(self.results_path).read()
        if not sheet_name:
        ...
        yaml_configuration.set_file_location(self.t2wml_spec_path)
        region, template, created_by = load_yaml_data(self.t2wml_spec_path, item_table, new_file_path, sheet_name)
