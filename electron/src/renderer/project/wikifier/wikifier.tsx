...
class Wikifier extends Component<WikifierProperties, WikifierState> {
  ...
  async handleDoCall(region: string, flag: string, context: string) {
    ...
    try {
      const json = await this.requestService.callWikifierService(wikiStore.project.pid, formData);
  ...
  async uploadEntitiesFile(event: any) {
    ...
    try {
      const json = await this.requestService.uploadEntities(wikiStore.project.pid, formData);
