...
class WikifierService:
    ...
    def wikify_region(self, cell_range:str, sheet, context=None):
        ...
        output=self._normalize_dataframe(output, row_offset, context, flattened_sheet_data, empty_vals)
    ...
    def _call_wikify_service(self, sheet_data, payload):
        with tempfile.TemporaryFile(mode='r+', newline="") as fp:
            ...
            response = requests.post(self.wikifier_endpoint, data=payload, files=files)
    ...
    def _get_errors(self, output, data, row_offset):
        empty_vals = np.where(pd.isnull(output))[0] #returns an array for row indices and col indices. 
